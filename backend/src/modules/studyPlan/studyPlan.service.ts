...
import {
  generateStudyPlanWithContext,
  generateTopicNotes,
  generateStudyPlanFromPDF,
} from "../ai/ai.service";
...
export const createStudyPlan = async (userId: number, studentName: string) => {
  ...
  // Check if any course has PDF content
  const pdfContent = courses
    .filter((c) => c.pdf_content)
    .map((c) => `=== ${c.name} ===\n${c.pdf_content}`)
    .join("\n\n");
  ...
  // Use PDF-aware generation if content exists
  const aiTasks = pdfContent
    ? await generateStudyPlanFromPDF(mappedCourses, studentName, pdfContent)
    : await generateStudyPlan(mappedCourses, studentName);
...
export const regenerateStudyPlan = async (
  ...
  if (currentPlan.rows.length > 0) {
   ...
   WHERE st.plan_id = $1 AND st.is_completed = true`,
    ...
    completedTopics = completedTasks.rows.map(
      (t) => `${t.course_name}: ${t.topic}`,
    );
